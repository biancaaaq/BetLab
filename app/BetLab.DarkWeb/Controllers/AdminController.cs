...
namespace BetLab.DarkWeb.Controllers
{
    public class AdminController : Controller
    {
        ...
        {
            var isAdmin = HttpContext.Session.GetString("Dark_IsAdmin");
            if (isAdmin != "true") return null;
            return HttpContext.Session.GetString("Dark_JwtToken");
        }
            ...
            decimal oddsHome, decimal oddsDraw, decimal oddsAway)
        {
            ...
            if (ok)
                TempData["AdminSuccess"] = "Eveniment adăugat cu succes.";
            else
                TempData["AdminError"] = "Eroare la adăugarea evenimentului.";
        ...
        public async Task<IActionResult> MarkWinningOutcome(int outcomeId, int eventId)
        {
            ...
            if (ok)
                TempData["AdminSuccess"] = "Cotă câștigătoare marcată.";
            else
                TempData["AdminError"] = "Eroare la marcarea cotei câștigătoare.";
        ...
        public async Task<IActionResult> SettleEvent(int eventId)
        {
            ...
            if (ok)
                TempData["AdminSuccess"] = "Eveniment decontat cu succes. Pariurile au fost procesate.";
            else
                TempData["AdminError"] = "Eroare la decontarea evenimentului. Verificați dacă cotele câștigătoare au fost marcate.";

            return RedirectToAction("Events");
        }

        [HttpPost]
        public async Task<IActionResult> GenerateMarkets(int eventId)
        {
            var token = GetAdminToken();
            if (token == null) return AccessDenied();

            var (ok, msg) = await _apiService.AdminGenerateMarketsAsync(token, eventId);
            TempData[ok ? "AdminSuccess" : "AdminError"] = msg;
            return RedirectToAction("Events");
        }

        [HttpPost]
        public async Task<IActionResult> GenerateAllMarkets()
        {
            var token = GetAdminToken();
            if (token == null) return AccessDenied();

            var (ok, msg) = await _apiService.AdminGenerateAllMarketsAsync(token);
            TempData[ok ? "AdminSuccess" : "AdminError"] = msg;
            return RedirectToAction("Events");
        }
