...
namespace BetLab.DarkWeb.Services
{
    public class ApiService
    {
        ...
        public async Task<EventPopularityViewModel?> GetEventPopularityAsync(int id)
        {
            try
            {
                var response = await _httpClient.GetAsync($"api/events/{id}/popularity");
                if (!response.IsSuccessStatusCode) return null;

                var json = await response.Content.ReadAsStringAsync();
                return JsonSerializer.Deserialize<EventPopularityViewModel>(json, _jsonOptions);
            }
            catch { return null; }
        }

        public async Task<DemoUserViewModel?> GetDemoUserAsync()
        {
            var response = await _httpClient.GetAsync("api/users/demo");
            if (!response.IsSuccessStatusCode)
                return null;

            var json = await response.Content.ReadAsStringAsync();
            return JsonSerializer.Deserialize<DemoUserViewModel>(json, _jsonOptions);
        }
    ...
    string? metadataJson)
        ...
        public async Task<CasinoSpinResponseViewModel?> SpinCasinoAsync(string token, int casinoGameId, decimal stake)
        {
            ...
            var json = await response.Content.ReadAsStringAsync();
            return JsonSerializer.Deserialize<CasinoSpinResponseViewModel>(json, _jsonOptions);
        }
            ...
            string email, string userName, string password, string cnp)
        {
            ...
            if (!response.IsSuccessStatusCode)
                return (null, json?.Trim('"') ?? "Eroare la înregistrare.");
        ...
        /// <summary>Cash Out anticipat — 65% din câștigul potențial.</summary>
            ...
            string token, int betSlipId, Guid userId)
        {
            using var request = new HttpRequestMessage(
                HttpMethod.Post, $"api/bets/{betSlipId}/cashout?userId={userId}&variant=Dark");
            request.Headers.Authorization =
                new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                return (false, null, null, err?.Trim('"') ?? "Eroare la cash out.");
            }
        ...
        {
            using var request = new HttpRequestMessage(
                HttpMethod.Get, $"api/bets/{betSlipId}/cashout-preview?userId={userId}&variant=Dark");
            request.Headers.Authorization =
                new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
            ...
            decimal oddsHome, decimal oddsDraw, decimal oddsAway)
        {
            ...
            var eventPayload = new
            {
                ...
                startTimeUtc, status = "Open", isLive = false
            };
            ...
            foreach (var outcome in outcomes)
            {
                ...
                req3.Content = new StringContent(JsonSerializer.Serialize(outcome), Encoding.UTF8, "application/json");
        ...
        public async Task<(bool Ok, string Message)> AdminGenerateMarketsAsync(string token, int eventId)
        {
            using var req = AdminRequest(HttpMethod.Post, $"api/admin/events/{eventId}/generate-markets", token);
            var resp = await _httpClient.SendAsync(req);
            if (!resp.IsSuccessStatusCode) return (false, "Eroare la generare piețe.");
            var json = await resp.Content.ReadAsStringAsync();
            using var doc = JsonDocument.Parse(json);
            int created = doc.RootElement.TryGetProperty("marketsCreated", out var mc) ? mc.GetInt32() : 0;
            return (true, $"Generat {created} piețe noi pentru eveniment.");
        }

        public async Task<(bool Ok, string Message)> AdminGenerateAllMarketsAsync(string token)
        {
            using var req = AdminRequest(HttpMethod.Post, "api/admin/events/generate-all-markets", token);
            var resp = await _httpClient.SendAsync(req);
            if (!resp.IsSuccessStatusCode) return (false, "Eroare la generare piețe pentru toate evenimentele.");
            var json = await resp.Content.ReadAsStringAsync();
            using var doc = JsonDocument.Parse(json);
            var msg = doc.RootElement.TryGetProperty("message", out var m) ? m.GetString() ?? "" : "OK.";
            return (true, msg);
        }
        ...
        public async Task<bool> SelfExcludeAsync(string token, string duration, string? reason = null)
        {
            using var req = new HttpRequestMessage(HttpMethod.Post, "api/me/self-exclude");
            req.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
            var payload = new { duration, reason };
            req.Content = new StringContent(JsonSerializer.Serialize(payload), Encoding.UTF8, "application/json");
            var resp = await _httpClient.SendAsync(req);
            return resp.IsSuccessStatusCode;
        }
